...
PRESENTATION=False
dataset_folder='/media/joey/DATASET/KITTI/dataset/sequences'
TIME_LIMIT_FOR_MULRAN=0
...
def evaluate_dataset(model, device, database_sets, query_sets):
    ...
    latent_vector_temp, _,positions,timestamp = get_latent_vectors(model, database_sets, device)
    ...
    latent_vector_temp, time_mean ,positions,timestamp= get_latent_vectors(model, query_sets, device)
    ...
    NP_final, NR_final, F1_final = get_pr_curve(database_information, query_information, query_sets,
                                                time_limit=TIME_LIMIT_FOR_MULRAN)
    pair_recall, pair_similarity, pair_opr = get_recall(database_embeddings, query_embeddings, query_sets,
                                                        database_sets)
...
def get_latent_vectors(model, set, device):
    ...
    for elem_ndx in tqdm.tqdm(set):
        x = load_pc(set[elem_ndx]["query"])[...,:3]
        p = np.asarray([set[elem_ndx]["northing"], set[elem_ndx]["easting"]])
        t = set[elem_ndx]["time_stamp"]
        with torch.no_grad():
...
def ReArrange(pc,order=[2,0,1]):
    temp=np.zeros_like(pc)
    temp[:,0]=pc[:,order[0]]
    temp[:, 1] = pc[:, order[1]]
    temp[:, 2] = pc[:, order[2]]
    return temp

def get_recall(database_vectors, query_vectors, query_sets, database_sets):
            ...
            if indices[0][j] in true_neighbors:
                if j == 0:
                    ...
                    if PRESENTATION:
                        ...
                        ### Save pc file ###
                        PC_query_temp = load_pc(Query_temp['query'],tensor=False)[:,:3]
                        # PC_query_temp=ReArrange(PC_query_temp)
                        PC_query_temp=Denoise_pc(PC_query_temp)
                        PC_query_filedir = os.path.join(Save_path_temp, 'Query_PC.ply')
                        PC_TopOne_temp = load_pc(TopOne_Temp['query'],tensor=False)[:,:3]
                        # PC_TopOne_temp = ReArrange(PC_TopOne_temp)
                        PC_TopOne_temp=Denoise_pc(PC_TopOne_temp)
                        PC_TopOne_filedir = os.path.join(Save_path_temp, 'TopOne_PC.ply')
                        # PC_TopTwo_temp = load_pc(TopTwo_Temp['query'],tensor=False)[:,:3]
                        # PC_TopTwo_temp = ReArrange(PC_TopTwo_temp)
                        ...
                        util.write_ply_rgb(PC_query_temp, colors=[0,0,0],out_filename=PC_query_filedir,one_color=True)
                        util.write_ply_rgb(PC_TopOne_temp, colors=[0,0,0],out_filename=PC_TopOne_filedir,one_color=True)
                        # util.write_ply_rgb(PC_TopTwo_temp, colors=[0,0,0],out_filename=PC_TopTwo_filedir,one_color=True)
...
if __name__ == '__main__':
    ...
    from model_v2.LWR_Core import LWRCore
    ...
    if torch.cuda.is_available():
        ...
        device = "cpu"
    resume_filename=os.path.join(cfg.weights_path,'MulRan_'+cfg.model+'_current_best.ckpt')
