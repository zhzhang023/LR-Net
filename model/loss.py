...
def make_loss():
    if cfg.loss == 'BatchHardTripletMarginLoss':
        # BatchHard mining with triplet margin loss
        # Expects input: embeddings, positives_mask, negatives_mask
        loss_fn = BatchHardTripletLossWithMasks(cfg.margin, cfg.normalize_embeddings)
    elif cfg.loss == 'BatchHardContrastiveLoss':
        loss_fn = BatchHardContrastiveLossWithMasks(cfg.pos_margin, cfg.neg_margin, cfg.normalize_embeddings)
    else:
        print('Unknown loss: {}'.format(cfg.loss))
